...
// CHANGED: previously a single module-level constant reused for the entire
// server lifetime. ROOT CAUSE of "JWT verification fails" (401) on variant
// playlist requests: each forceFresh getSession() call gets a brand NEW
// session/JWT from Pluto's boot API, but we kept sending the SAME deviceId
// across every one of those sessions. Pluto's backend appears to tie the
// JWT to the specific deviceId/sid that requested it — reusing an old
// deviceId with a new JWT, or vice versa, fails verification. Now we
// generate a fresh deviceId for every playback session.
function generateDeviceId() {
  return 'reddawn-' + Math.random().toString(36).slice(2) + Date.now().toString(36);
}
const STATIC_DEVICE_ID = generateDeviceId(); // used only for the long-lived catalog/metadata fetch
...
async function getSession(forceFresh) {
  ...
  // Each fresh playback session gets its OWN deviceId, matching the new JWT
  // Pluto is about to issue us. The cached (non-forceFresh) catalog session
  // keeps using the static one — that path doesn't hit the JWT-verified
  // stitcher endpoint, so it doesn't need to change.
  const deviceId = forceFresh ? generateDeviceId() : STATIC_DEVICE_ID;
  ...
  // Pull real device coordinates from the boot session. ROOT CAUSE FOUND:
  // we were never sending deviceLat/deviceLon to the stitcher at all. The
  // stitcher's own relative variant-playlist URLs came back with
  // deviceLon=83 (POSITIVE, near India) instead of -83 (Columbus, OH,
  // correct), which explains every random-language filler video we saw
  // throughout this debugging session. We now grab the real coordinates
  // Pluto's own boot response computed for our clientIP, and pass them on
  // every stitcher request ourselves instead of letting Pluto's stitcher
  // guess/default them on its own.
  const deviceLat = session.deviceLat != null ? session.deviceLat : 39.9700012;
  const deviceLon = session.deviceLon != null ? session.deviceLon : -83; // MUST be negative for US East/Central

  const sessionObj = { token, deviceId: deviceId, fetchedAt: Date.now(), stitcherHost, countryCode: region, deviceLat, deviceLon };
  if (!forceFresh) {
...
function buildStreamUrl(channelId, token, deviceId, stitcherHost, countryCode, deviceLat, deviceLon) {
  const params = new URLSearchParams({
    ...
    deviceDNT:         '0',
    deviceId:          deviceId,
    deviceLat:         deviceLat != null ? String(deviceLat) : '39.9700012',
    deviceLon:         deviceLon != null ? String(deviceLon) : '-83',
    deviceMake:        'chrome',
    deviceModel:       'web',
    deviceType:        'web',
    deviceVersion:     '124.0.0',
  ...
  return `${host}/v2/stitch/hls/channel/${channelId}/master.m3u8?${params.toString()}`;
}
...
async function getFreshStreamUrl(channelId) {
  ...
  const { token, deviceId, stitcherHost, countryCode, deviceLat, deviceLon } = await getSession(true);
  const url = buildStreamUrl(channelId, token, deviceId, stitcherHost, countryCode, deviceLat, deviceLon);
  // Return the token too: Pluto's variant-playlist/segment requests appear
  // to require the JWT as an Authorization header (the same way the
  // channel-list API does), not just as a query param on the master
  // playlist URL. index.js needs this to attach the header on every
  // downstream /proxy/segment fetch.
  return { url, token };
}
