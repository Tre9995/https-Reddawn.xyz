...
// authToken (optional) gets appended as &authToken=... on every proxied
// link so /proxy/segment can attach it as an Authorization header — Pluto's
// variant playlist/segment endpoints return 401 "JWT verification fails"
// without it, since the JWT isn't embedded in the stitcher URL itself.
function rewriteM3U8(m3u8, sourcePlaylistUrl, baseUrl, authToken) {
  var lines = m3u8.split('\n');

  function proxyLink(absoluteUrl) {
    var link = baseUrl + '/proxy/segment?url=' + encodeURIComponent(absoluteUrl);
    if (authToken) {
      link += '&authToken=' + encodeURIComponent(authToken);
    }
    return link;
  }
...
app.get('/proxy/stream/:channelId.m3u8', async function (req, res) {
  ...
  try {
    ...
    var fresh = await pluto.getFreshStreamUrl(channelId);
    var freshStreamUrl = fresh.url;
    var authToken = fresh.token;
    ...
    var baseUrl = getBaseUrl(req);
    // Pass authToken through so every rewritten /proxy/segment URL carries
    // it as a param — ROOT CAUSE of "JWT verification fails" (401): variant
    // playlist/segment requests need the JWT as an Authorization header,
    // same as the channel-list API, but /proxy/segment never had access to
    // it since it only receives the bare stitcher URL via ?url=.
    var rewritten = rewriteM3U8(m3u8, freshStreamUrl, baseUrl, authToken);
...
// Extracts the Pluto channel ID from a stitcher URL like
// https://.../v2/stitch/hls/channel/<channelId>/1042180/playlist.m3u8
// so we can fetch a brand-new session/token if the current one expires
// mid-playback.
function extractChannelIdFromStitcherUrl(url) {
  var match = url.match(/\/channel\/([a-zA-Z0-9]+)\//);
...
app.get('/proxy/segment', async function (req, res) {
  ...
  async function doFetch(fetchUrl, token) {
    return axios.get(fetchUrl, { timeout: 30000, responseType: 'arraybuffer', headers: buildHeaders(token) });
  }

  try {
    var response;
    var effectiveUrl = url;
    var effectiveToken = authToken;

    try {
      response = await doFetch(effectiveUrl, effectiveToken);
    } catch (firstErr) {
      var firstStatus = (firstErr.response && firstErr.response.status) || null;
      // MID-PLAYBACK FIX: live HLS players keep re-requesting segments using
      // the SAME authToken embedded when the master playlist was first
      // fetched, but that JWT expires after a few minutes (matches the
      // "kicks back to catalog after 2-5 minutes" symptom). On a 401, fetch
      // a brand-new session/token for the same channel and retry once
      // in-place, instead of failing and forcing the player to give up.
      if (firstStatus === 401) {
        var channelId = extractChannelIdFromStitcherUrl(url);
        if (channelId) {
          console.log('[proxy/segment] 401 detected mid-stream, refreshing token for channel ' + channelId);
          var fresh = await pluto.getFreshStreamUrl(channelId);
          effectiveToken = fresh.token;
          response = await doFetch(url, effectiveToken);
        } else {
          throw firstErr;
        }
      } else {
    ...
    if (contentType.indexOf('mpegurl') !== -1 || url.indexOf('.m3u8') !== -1) {
      ...
      var baseUrl = getBaseUrl(req);
      var rewritten = rewriteM3U8(text, url, baseUrl, effectiveToken);
      res.setHeader('Content-Type', 'application/vnd.apple.mpegurl');
      res.setHeader('Access-Control-Allow-Origin', '*');
