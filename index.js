...
// Direct network test - bypasses pluto.js completely
app.get('/debug/nettest', async function (req, res) {
  try {
    console.log('[nettest] fetching i.mjh.nz...');
    var r = await axios.get('https://i.mjh.nz/PlutoTV/us.json', { timeout: 15000 });
    var keys = Object.keys(r.data || {});
    console.log('[nettest] success, keys:', keys.join(','));
    sendJson(res, { ok: true, status: r.status, keys: keys, sample: keys.slice(0, 5) });
  } catch (e) {
...
app.get('/debug/categories', async function (req, res) {
  console.log('[debug] /debug/categories called');
  try {
    var categories = await pluto.getCategories();
    console.log('[debug] categories returned:', categories.length);
    sendJson(res, { count: categories.length, sample: categories.slice(0, 3) });
  } catch (e) {
    console.error('[debug] categories error:', e.message);
    sendJson(res, { count: 0, error: e.message });
  }
});

app.get('/debug/search', async function (req, res) {
  console.log('[debug] /debug/search called');
  try {
    var q = req.query.q || 'batman';
    var items = await pluto.search(q);
    console.log('[debug] search returned:', items.length);
