...
async function getSession(forceFresh) {
  ...
  const sessionObj = { token, deviceId: DEVICE_ID, fetchedAt: Date.now(), stitcherHost, countryCode: region };
  cacheSet('session', sessionObj, SESSION_TTL_MS);
  return sessionObj;
}

// ─── Build a working HLS stream URL ──────────────────────────────────────────
function buildStreamUrl(channelId, token, deviceId, stitcherHost, countryCode) {
  // UPDATED based on a real, recently-observed (March 2026) working Pluto
  // web request: Pluto now uses a /v2/stitch/ path (not /stitch/), and the
  // real web client sends both `sessionID`/`sid` (matching) and the JWT
  // itself as a `jwt` param, plus `country`/`marketingRegion`, capitalized
  // `deviceMake=chrome` consistently, `advertisingId` (empty but present),
  // and `app_name` alongside `appName`. Our old request was missing several
  // of these, which may be why the stitcher accepted the request (200 OK,
  // valid-looking master.m3u8) but served a generic "device not supported"
  // filler instead of real programming once it got to actual content
  // resolution deeper in the pipeline.
...
async function getFreshStreamUrl(channelId) {
  ...
  const { token, deviceId, stitcherHost, countryCode } = await getSession(true);
  return buildStreamUrl(channelId, token, deviceId, stitcherHost, countryCode);
}
